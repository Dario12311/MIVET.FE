    ...
    switch (this.seccionActual) {
      case 1:
        if (
          ...
          this.mensajeError =
            'Por favor, complete todos los campos correctamente. Las contraseñas deben coincidir.';
          esValido = false;
        } else {
          this.mensajeError = '';
        }
    ...
    this.loginService.login(this.nombreUsuario, this.password).subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          ...
          this.mensajeError =
            'Error al iniciar sesión. Por favor, intenta nuevamente más tarde.';
        }
    ...
    this.registroService.registrarUsuario(this.registroForm.value).subscribe({
      ...
      error: (error: any) => {
        ...
        if (error.status === 404) {
          ...
          this.mensajeError =
            'Error al registrarse. Por favor, intenta nuevamente más tarde.';
        }
      },
    });
  }
